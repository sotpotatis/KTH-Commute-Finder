...
<script>
    ...
    let classesToApply = [BADGE_COLORS_TO_CLASSES[color],
BADGE_SIZES_TO_CLASSES[size], "font-bold", circular ? "rounded-full": "rounded-lg",
    "px-3 py-3 w-fit"]
    if (isClickable){ // Add button-related classes if clickable
        classesToApply.push(...[
            !isDisabled ? "hover:cursor-pointer": "hover:cursor-not-allowed"
            , "hover:opacity-70",
        "select-none"])
    }
    if (flexbox){ // Render as flexbox if set
