...
<script>
    ...
    $: isCollapsed = true;
    $: isCollapsible = partType === "publicTransport" && partData.stops !== undefined && partData.stops.length > 2
...
<div class="bg-gray-900/50 backdrop-blur-xl rounded-lg my-3 p-3 grid grid-cols-5 md:grid-cols-4 gap-x-8 items-center">
    <!-- Add time -->
...
{#if isCollapsible}
    <p class="text-center">
    <Button color="link" on:click={()=>{isCollapsed = !isCollapsed}}>
