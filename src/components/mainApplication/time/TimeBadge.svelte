...
<script>
    ...
    const destinationStationArrival = DateTime.fromISO(
        lastTripStep.origin.time
    )  // When we arrive to the destination station
    ...
    if (overridenWalkingTime === null){
        destinationArrival = DateTime.fromISO(
            lastTripStep.destination.time
        )
    }
    else {
...
{#if isAfterStartTime || isAq}
    ...
    <!-- Show popup with information -->
        {#if showPopup}
            <Popup showPopup={true} on:popupClosed={()=>{
            console.log("Closing popup...")
               showPopup = !showPopup
                ...
                <svelte:fragment slot="popup-body">
                    {#if isAq}
                    <p>Denna ankomsttid är under den <i>akademisk kvarten</i> (föreläsningar, övningar etc. är utsatta att börja
                    på heltimme i schemat, men börjar i själva verket kvart över). Försäkra dig om att
                    detta schemaevent verkligen har akademisk kvart!</p>
                    <p>Akademisk kvart gäller <b>INTE</b> på tentor och liknande!</p>
                    {:else}
                        <p>Denna ankomsttid är efter schemaeventet börjar, inklusive akademisk kvart. Med andra ord, du kommer med största sannolikhet för sent om du tar denna resa!</p>
                    {/if}
                </svelte:fragment>
            </Popup>
        {/if}
