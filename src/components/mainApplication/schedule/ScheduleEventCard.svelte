...
<script>
	...
	$: parsedStartTimeText = DateTime.fromISO(startTime)
...
<button
	class={passClassList([
		'bg-gray-900/50 hover:bg-gray-900/30 backdrop-blur-xl hover:skew-y-[-1deg] w-72  rounded-lg py-1 my-6 px-3 mx-3 text-white',
...
>
		...
		<h2 class="font-black text-2xl">
			...
			{#if eventType !== null}
				<span
					...
					class={passClassList([
						EVENT_TYPE_TO_COLOR_FLAIR_CLASSES[eventType.id],
	...
	{#if isDisabled}
		<p class="font-bold flex flex-row text-sm gap-x-2 items-center py-3">
			<Icon icon="material-symbols:warning" />Plats kunde inte hittas. Sök resa manuellt.
		</p>
	{/if}
</button>
