...
<script>
    ...
    onMount(()=>{
        if (scheduleURL !== null && scheduleURL !== undefined && scheduleEvents === null){
            ...
            loading = true
            error = false
            api.getScheduleEvents(scheduleURL, scheduleSearchDates.startDate, scheduleSearchDates.endDate).then(([requestSuccessful, responseData])=>{
    ...
    $: currentPickedDate = scheduleEvents !== null  ? (scheduleEventDays.length > 0 ?  scheduleEventDays[currentPickedDayIndex-1] : null): null
    ...
    $: currentPickedDay = currentPickedDate !== null ? DateTime.fromISO(currentPickedDate).setLocale("sv-se") : null
    $: currentPickedDayText = currentPickedDay !== null ? currentPickedDay.setLocale("sv-se").toFormat("EEEE, dd MMM").toLowerCase(): null
...
{:else if error}
    {#if scheduleEventDays.length === 0} <!-- If no schedule events were found -->
        <Error message="Inga schemaevent inom den närmsta veckan hittades. Är det tentaplugg nu måntro?"/>
    {:else}
        ...
        {#each Object.values(groupedScheduleEvents) as hourScheduleEvents}
            ...
            <ScheduleEventCardGroup numberOfEvents={hourScheduleEvents.length}>
                <svelte:fragment slot="first-event">
                    <ScheduleEventCard eventTitle={hourScheduleEvents[0].summary.title}
                                           eventType={hourScheduleEvents[0].summary.type}
                                            eventLocation={hourScheduleEvents[0].location}
                                           startTime={hourScheduleEvents[0].start} on:click={()=>{pickScheduleEvent(hourScheduleEvents[0])}}/>
                </svelte:fragment>
                <svelte:fragment slot="collapsible-events">
                    {#each hourScheduleEvents.slice(1,hourScheduleEvents.length) as scheduleEvent}
                            <ScheduleEventCard eventTitle={scheduleEvent.summary.title}
                                               eventType={scheduleEvent.summary.type}
                                                   eventLocation={scheduleEvent.location}
                                               startTime={scheduleEvent.start} on:click={()=>{pickScheduleEvent(scheduleEvent)}}/>
                    {/each}
                </svelte:fragment>
