...
<script>
    ...
    $: walkTime = null
    const getOverridenWalkingTimes = ()=>{return settings.get("overridenWalkingTimes", {})}
    ...
    $: if (selectedTrip !== null){
        correctedWalkTime = overridenWalkingTimes[`${lastTripPart.origin.slID}-${destinationRoom.information.location.buildingName}`]
        if (correctedWalkTime === undefined){ // If the user has not overriden the walking time
            correctedWalkTime = null
        }
        walkTime = correctedWalkTime === null ? Duration.fromISO(lastTripPart.walkTime).minutes : correctedWalkTime
    }
    else {
        walkTime = null
    }
    ...
    const findTrips = () => {
        ...
        if (settings.avoidedTravelMethods !== undefined && settings.avoidedTravelMethods !== null && settings.avoidedTravelMethods.length > 0){
            ...
            for (const [travelMethodKey] of Object.entries(API_TRAVEL_METHOD_TYPES)){
                if (!settings.avoidedTravelMethods.includes(travelMethodKey)){ // If not ignored, add to list
        ...
        }, walkTime, includedTravelMethods).then(([requestSuccessful, responseData])=>{
...
{:else if error}
    {#if foundTrips !== null && foundTrips.length === 0}
        <Error message="Tyvärr, inga resor kunde hittas. Testa att komma tillbaka senare eller ändra i din sökning."/>
        ...
        <div class="text-center leading-loose">
            ...
            <ul class="list-disc list-inside">
                ...
                <li><i>Ligger eventet utanför huvudcampus?</i> Exempelvis, för KTH Valhallavägen så saknas platsuppgifter för vissa laborationssalar
                som ligger i Alba Nova.</li>
                <li>Är du säker att du inte valt att söka från t.ex. en busstation, men har <i>inaktiverat</i> buss som färdmedel (se inställningsmenyn)</li>
                <li>Tekniska störningar hos SL, hos KTH's schema- eller platssökning, eller hos ägaren av denna tjänst.</li>
                <li>En bugg.</li>
            </ul>
        </div>
    {:else}
    ...
    passClassList([
        ...
        <div class="snap-normal snap-center" data-associatedTrip={JSON.stringify(foundTrip)}>
            <TimeBadge
                ...
                tripData={foundTrip}
                scheduleEventStart={
                    DateTime.fromISO(scheduleEvent.start)
                }
                on:click={()=>{
                    selectedTrip = foundTrip
                    console.log("Selected trip updated to: ", selectedTrip)
            }}/>
            <TripSummary tripData={foundTrip}
                         iconBackground={true}
