...
<script>
    ...
    const roomPicked = (roomData)=>{console.log("Room picked. Dispatching event...", roomData)
                    dispatch("locationPicked", {location: roomData})}
    onMount(()=>{
        if (availableRooms.length === 1 && !dataNotAvailableForAnyRoom){ // Automagically move forward if the event only has one room
            // and it is available
...
<p class="text-small">Osäker? Välj bara ett godtyckligt rum som ligger i den byggnad som du vill gå till.</p>
{/if}
{#if dataNotAvailableForAnyRoom}
    <!-- Show if no locations could be found -->
    <TextAndWarning>
        Plats kunde inte hittas för eventets rum
    </TextAndWarning>
    <p class="text-sm">Du behöver tyvärr söka din resa manuellt. Detta kan inträffa exempelvis om du har ett
    schemaevent som inte ligger på huvudcampus. För Campus Valhallavägen, så kan t.ex. laborationer vid Alba Nova
    trigga detta fel.</p>
{:else if dataNotAvailableForARoom}
    <!-- Show if a location could not be found for the room -->
    <TextAndWarning>
        Plats kunde inte hittas för ett eller flera rum
    </TextAndWarning>
    <p class="text-sm">Plats kunde inte hittas för ett eller flera rum.
    Sök din resa manuellt för dessa. De har markerats med <b>grå</b> bakgrund.</p>
{/if}
<div class="grid grid-cols-3 md:grid-cols-5 text-center py-3">
    {#each availableRooms as roomData}
        <div>
            {#if roomData.information !== null}
                <Badge color={roomData.dataAvailable ? "purple": "gray"} isClickable={
                true
                }
               isDisabled={!roomData.dataAvailable} on:click={roomData.dataAvailable ? ()=>{
                    roomPicked(roomData)
                }: undefined} text={roomData.information.roomName}>
                </Badge>
            {/if}
        </div>
    {/each}
