...
<div class="text-center">
    ...
    {#if numberOfEvents > 1}
        <button class="text-white font-bold" on:click={()=>{isCollapsed = !isCollapsed}}><span class="animate-pulse text-orange-400">●</span>
            {isCollapsed ? `+${numberOfEvents-1} ${numberOfEvents  >2 ? "händelser": "händelse"}`: "Gruppera händelser"}</button>
    {/if}
