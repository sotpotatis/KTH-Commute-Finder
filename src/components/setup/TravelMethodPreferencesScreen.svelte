...
<script>
	...
	$: onlyOneTravelMethodAvailable =
		avoidedTravelMethods.length === Object.keys(API_TRAVEL_METHOD_TYPES).length - 1;
</script>
...
{#each Object.entries(API_TRAVEL_METHOD_TYPES) as [travelMethodId, travelMethodInfo]}
	...
	<Button
		extraClasses={isAvoided ? ['line-through'] : null}
		size={!compactMode ? 'big' : 'medium'}
		multipleItems={true}
		circular={true}
		color={isAvoided ? 'gray' : 'blue'}
	...
	>
		...
		<Icon
			icon={!travelMethodIsAvoided(travelMethodId)
