...
<script>
	...
	import {
		...
		handleSavedStationsSetting,
		handleScheduleLinkSetting
	...
	const SETUP_STEPS = [
		{
			...
			nextButtonLocked: () => {
				return settings == null || settings.scheduleURL === null;
			}
		},
		{
			id: 'add_stations',
			...
			nextButtonLocked: () => {
				return settings == null || settings.savedStations === null;
			},
		...
		},
		{
			...
			title: 'Klart!',
			description:
				'Nu är det bara att välja schemaevent och ankomsttid så sköter hemsidan resten. 💅',
			subdescription:
				'Om du någonsin vill ändra dina inställningar, titta bara efter inställningsikonen i nedre högra hörnet! (💅 x2)'
		}
	];
	...
	$: nextButtonLocked =
		activeSetupScreen.nextButtonLocked !== undefined
			? activeSetupScreen.nextButtonLocked()
			: !activeSetupScreen.isSkippable && setupStep !== SETUP_STEPS.length;

	const dispatch = createEventDispatcher(); // So we can dispatch event for when setup is done
	// Create functions
	const DISPATCH_FINAL_STEP_DONE = () => {
		// For dispatching final step
		console.log('Dispatching that the final step is done.');
		dispatch('setupDone');
	};
	const NEXT_STEP = () => {
		if (nextButtonLocked) {
			// Prevent lock from changing screen
			return;
		}
		if (setupStep < TOTAL_SETUP_STEPS) {
			...
			throw new Error(
				'Will not go to the next step - you are already at the last step! Please use the DISPATCH_FINAL_STEP_DONE function instead.'
			);
		}
	};
	const PREVIOUS_STEP = () => {
	...
	onMount(() => {
		...
		if (settings !== null && settings.scheduleURL !== null) {
			...
			if (settings.savedStations !== null) {
				...
				setupStep += 1;
				if (settings.avoidedTravelMethods !== null) {
					// If we get here, we are done with setup
...
see the defaults in the TitleAndDescription component -->
	...
	{#if activeSetupScreen.id === 'enter_schedule_link'}
		<ScheduleLinkInputScreen
			filledInLink={settings.scheduleURL}
			on:schedulelink={(event) => {
	...
	{#if activeSetupScreen.id === 'add_stations'}
		<StationInputScreen
			stations={settings.savedStations !== null ? settings.savedStations : []}
			on:stationsUpdated={(event) => {
	...
	{#if activeSetupScreen.id === 'select_travel_methods'}
		<TravelMethodPreferencesScreen
			avoidedTravelMethods={settings.avoidedTravelMethods !== null
				? settings.avoidedTravelMethods
				: []}
			on:avoidedTravelMethodsUpdated={(event) => {
		...
		/>
	{/if}
	<div class={passClassList(['grid', 'gap-x-2', setupStep > 1 ? 'grid-cols-2' : 'grid-cols-1'])}>
		...
		<Button
			...
			on:click={!isLastSetupStep ? NEXT_STEP : DISPATCH_FINAL_STEP_DONE}
			>{isLastSetupStep ? 'Klar' : 'Nästa'}</Button
		>
	</div>
		...
		<Button
			...
			on:click={() => {
				...
				if (activeSetupScreen.linkedSettingKey !== undefined) {
					...
					const settingsValueOnSkip =
						activeSetupScreen.settingValueOnSkip !== undefined
							? activeSetupScreen.settingValueOnSkip
							: null;
