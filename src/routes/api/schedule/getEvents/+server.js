...
import {
	extractEvents,
	retrieveSchedule
} from '../../../../lib/scheduleRetrieval/retrieveSchedule.js';
...
export async function GET({ request }) {
	...
	if (urlParametersValid) {
		...
		const optionalURLParameters = getOptionalURLParameters(
			optionalURLParameterSettings,
			urlParameters
		);
		// Validate that these extra URL parameters are valid if they are user-passed
		for (const [optionalParameter, defaultParameterValue] of Object.entries(
			optionalURLParameterSettings
		)) {
			if (optionalURLParameters[optionalParameter] !== defaultParameterValue) {
				console.log(`Validating user-passed ${optionalParameter}...`);
				const optionalURLParameterDateTime = DateTime.fromISO(
					optionalURLParameters[optionalParameter]
				);
				...
				if (!optionalURLParameters[optionalParameter].isValid) {
					...
					return generateResponse(
						API_STATUS_ERROR,
						{
							message: `Invalid value of the parameter \"${optionalParameter}\" supplied. Please check it.`
						},
						400
					);
				} else {
					...
					optionalURLParameters[optionalParameter] =
						optionalParameter === 'startDate'
							? optionalURLParameterDateTime.startOf('day')
							: optionalURLParameterDateTime.endOf('day');
				}
			}
		}
		...
		try {
			...
			if (schedule !== null) {
				...
				const scheduleEvents = await extractEvents(
					schedule,
					optionalURLParameters.startDate,
					optionalURLParameters.endDate
				);
