...
export function generateResponse(status, responseData, statusCode=null){
    if (statusCode == null){ // Fill out default status code
        statusCode = 200
    }
...
export function validateURLParameters(requiredURLParameters, request){
    ...
    return [(foundURLParameters.length === requiredURLParameters.length),
        urlParameters]
}
/**
...
export function getOptionalURLParameters(optionalURLParameters, providedURLParameters){
    ...
    for (const [parameterName, defaultValue] of Object.entries(optionalURLParameters)){
        urlParameters[parameterName] = (
            providedURLParameters.has(parameterName) ? // If parameter exists
            providedURLParameters.get(parameterName): defaultValue)
    }
...
// requests.
export const MISSING_REQUIRED_ARGUMENTS = ()=> {return generateResponse(API_STATUS_ERROR, {
...
export const SCHEDULE_RETRIEVAL_ERROR_RESPONSE = () => {return generateResponse(API_STATUS_ERROR, {
    success: false,
    message: "Schedule request and/or parse failed."
}, 500)}
export const GENERIC_SERVER_ERROR = () => {return generateResponse(API_STATUS_ERROR, {
    success: false,
