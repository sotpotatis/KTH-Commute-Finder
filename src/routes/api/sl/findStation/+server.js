...
export async function GET({request}){
    ...
    if (urlParametersValid){
        ...
        try {
            ...
            if (searchResults.StatusCode !== 0){ // 0 means that the request succeeded (apparently)
            ...
            for (const foundStation of searchResults.ResponseData){
                ...
                else { // Since the SL API defaults to stations only, this is unexpected
                    console.warn(`SL API returned an entry with a type that was not "Station": ${JSON.stringify(foundStation)}.`)
                }
            }
