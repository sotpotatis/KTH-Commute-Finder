...
export function updateUserSettings(newSettingsValue) {
	if (browser) {
		localStorage.setItem(USER_SETTINGS_LOCAL_STORAGE_KEY, JSON.stringify(newSettingsValue));
...
export function getSetting(settingsKey) {
	...
	if (currentUserSettings === null) {
		...
		// Return the current value of the setting if it exists
...
export class Settings {
	...
	announceChanges() {
		console.debug(
			`Announcing changes to ${this.subscribedFunctions.length} subscribed functions...`
		);
