...
import {
	...
	retrieveRawSchedule,
	retrieveSchedule
} from '../scheduleRetrieval/retrieveSchedule.js';
...
const kthPlacesAPI = new KTHPlacesAPI(
	import.meta.env.VITE_KTH_PLACES_API_KEY,
	import.meta.env.VITE_KTH_PLACES_API_USER_AGENT
);
...
function getKeyReturnValue(key, keyData, keyReference, updateInterval) {
	...
	if (keyData !== null && keyData !== undefined) {
		...
		if (DATABASE_TYPE !== 'memory' && now > syncedAtDateTime.plus({ minutes: updateInterval })) {
			console.log(
				`Asking for cached key "${key}" to be updated (last synced: ${syncedAtDateTime})`
			);
			...
			// If the key has been recently synced
			return [keyData.value, keyReference];
		}
	} else {
...
export async function getCachedKeys(keyType, keys, updateInterval) {
	...
	if (DATABASE_TYPE !== null && DATABASE_TYPE !== 'fauna') {
		...
		// Use specific querying for Fauna
		// Execute the query
		const keyQueryResults = await database.query(
			...
			query.Map(
				...
				query.Lambda(
					'key',
					query.Map(
						query.Paginate(query.Match(query.Index(`${keyType}-keys`), query.Var('key'))),
						(reference) => query.Get(reference)
					)
		...
		for (let i = 0; i < keyQueryResults.length; i++) {
			...
			if (keySearchResult.data.length === 0) {
...
export async function setCachedKey(keyType, key, previousKeyInstance, newValue, ttl) {
	...
	if (DATABASE_TYPE === 'deta') {
		...
		if (previousKeyInstance !== null) {
			// If existent in database
	...
	console.log(
		`Data for ${fullKey} successfully updated and stored in cache (sync time: ${DateTime.fromSeconds(
			newValue.syncedAt
		)}).`
	);
}
...
export async function getRoomsData(roomNames) {
	...
	let roomDatas = {};
	const cachedKeysResults = await getCachedKeys(
		'kthPlace',
		allRoomNames,
		KTH_ROOMS_UPDATE_INTERVAL
	);
...
export async function getCachedSchedule(scheduleURL) {
	...
	let [cachedKeyValue, cachedKey] = await getCachedKey(
		scheduleDatabaseKey,
		KTH_SCHEDULE_UPDATE_INTERVAL
	);
	...
	if (cachedKeyValue === null) {
		...
		// Load raw schedule data from cache
