...
export class KTHPlacesAPI {
	...
	async findRoom(roomName) {
		...
		if (Object.keys(responseData).length === 0) {
			console.warn(
				`Requested KTH room ${roomName} not found (response: ${JSON.stringify(responseData)}).`
			);
			return null;
		} else {
...
export function findBuildingsFromRooms(roomsData) {
	...
	for (const [roomName, roomData] of Object.entries(roomsData)) {
		...
		if (
			roomData.dataAvailable &&
			roomData.information.location !== undefined &&
			roomData.information.location.buildingName !== undefined
		) {
