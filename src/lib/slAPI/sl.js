...
import {POINT_TYPE_VIA, POINT_TYPE_DESTINATION, POINT_TYPE_ORIGIN, POINT_TYPE_AVOID} from "./reseplanerareClasses.js";
export const PRODUCT_TYPE_TO_API_TYPE = { // Convert SL product types to a format that I like better
MET: {type: "metro",
...
export class SLAPIWrapper {
    ...
    async sendRequest(apiKey, apiService, urlParameters) {
        ...
        if (apiService === "lookahead"){ // The lookahead service has migrated to a new URL. Use that.
    ...
    async platsuppslag(query, stationsOnly=null, maxResults=null){
        ...
        const responseJSON = await this.sendRequest(this.apiKeys.platsuppslag, "typeahead", urlParameters)
        // You can get a 200 even if you get an error, interesting...
        if (responseJSON.StatusCode !== 0){
            // Look for an error message
            const APIErrorMessage = responseJSON.Message !== undefined ? responseJSON.Message : "The API did not provide an error message."
     ...
     * @return {Promise<*>}  Await me and you will (hopefully) get a JSON response from the API.
     */
    async reseplanerare(includedPoints, searchDate=null, searchTime=null, tripsBeforeStartTime=0, tripsAfterStartTime=null,
                        arrivalAtSearchTime=null,
                        products=null,
                        includePassedStations=null,
                        minChangeTime=null, maxChangeTime=null, avoidArgs=[], viaArgs=[],
                        context=null){
        let urlParameters = {
        ...
        if (products !== null){
            ...
            for (const product of products){
                if (Object.keys(API_TRAVEL_METHOD_TYPES).includes(product)){
                    ...
                    console.warn(`Invalid argument passed to trip planner: ${product} doesn't seem to be a valid product!`)
                }
            }
        ...
        for (const includedPoint of includedPoints){
            ...
            if (includedPoint.pointType === POINT_TYPE_ORIGIN || includedPoint.pointType === POINT_TYPE_DESTINATION){
        ...
        const responseJSON = await this.sendRequest(this.apiKeys.reseplanerare, "/TravelplannerV3_1/trips.json", urlParameters)
        ...
        if (responseJSON.errorCode !== undefined || responseJSON.errorText !== undefined){
            ...
            let errorInformation = responseJSON.errorCode !== undefined ? `Error code: ${responseJSON.errorCode}`: "Error code not available."
            errorInformation += responseJSON.errorText !== undefined ? ` Error text: ${responseJSON.errorText}`: " Error text not available."
