...
export class Coordinate {
	...
	constructor(pointType, latitude, longitude) {
		if (pointType === POINT_TYPE_VIA) {
			throw new Error(
				'POINT_TYPE_VIA is not supported for this type of object (it is only supported for stations).'
			);
		}
...
export class Station {
	...
	toObject() {
		if ([POINT_TYPE_VIA, POINT_TYPE_AVOID].includes(this.pointType)) {
			// Via or Avoid is not convertible to object
			throw new Error(
				'The Station is marked as a Via or Avoid travel location. It is not convertible to an object!'
			);
		}
