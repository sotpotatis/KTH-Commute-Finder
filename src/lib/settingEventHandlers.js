...
export function handleAvoidedTravelMethodsSetting(event, settings){
    ...
    const changeForbidden = newAvoidedTravelMethods.length === Object.keys(API_TRAVEL_METHOD_TYPES).length
    if (!changeForbidden){ // If the user hasn't disabled all the travel methods
